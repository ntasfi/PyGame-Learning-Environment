...
class RayCastPlayer():
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if key == self.actions["right"]:
                    ...
                    _planeX = self.plane[0,0]*X_TURN - self.plane[0,1]*Y_TURN
                    _planeY = self.plane[0,0]*Y_TURN + self.plane[0,1]*X_TURN
                ...
                if key == self.actions["left"]:
                    ...
                    _dirX = self.dir[0,0]*X_INV_TURN - self.dir[0,1]*Y_INV_TURN
                    _dirY = self.dir[0,0]*Y_INV_TURN + self.dir[0,1]*X_INV_TURN

                    _planeX = self.plane[0,0]*X_INV_TURN - self.plane[0,1]*Y_INV_TURN
                    _planeY = self.plane[0,0]*Y_INV_TURN + self.plane[0,1]*X_INV_TURN
    ...
    def draw(self):
        #N = width/resolution
        #N,2
        cameraX = np.arange(0.0, self.width, self.resolution).astype(np.float32)[:,np.newaxis]
        ...
        delta_dist = np.sqrt( 1.0 + np.array([ 1.0/(ray_div), ray_div ]) ).T #N,2
        ...
        side_dist, delta_dist, map_, side = self._DDA(side_dist, delta_dist, map_, step)
    ...
    def _DDA(self, side_dist, delta_dist, map_, step):
        ...
        while np.sum(hits) < side_dist.shape[0]:
            ...
            hits = np.logical_or( hits, (self.map_[ map_[:, 0], map_[:, 1] ] > 0)[:, np.newaxis] ) 
...
if __name__ == "__main__":
    ...
    map_grid[ map_grid > 0 ] = np.random.randint(2, high=5, size=map_grid[ map_grid > 0 ].shape)
    ...
    while True:
        ...
        for i in range(len(c)):
            pygame.draw.line(screen, (col[i][0], col[i][1], col[i][2]), (c[i], t[i]), (c[i], b[i]), rc.resolution)
